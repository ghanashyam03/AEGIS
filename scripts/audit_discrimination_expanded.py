# ruff: noqa: E501
"""Diagnostic script for Baseline Classifier Discrimination at scale (Step 0).

Measures ROC-AUC and PR-AUC for:
- Kilonova (class 64) vs Rest (classes 90, 95)
- SLSN-I (class 95) vs Rest (classes 64, 90)
on the expanded evaluation population (55,915 objects) across epochs e in {0.0, 2.0, 7.0} days.
...
def main() -> None:
    t0 = time.time()
    print("=== AEGIS STEP 0: SCALE-UP DISCRIMINATION DIAGNOSTIC ===", flush=True)
    ...
    # 2. Load expanded evaluation cohort metadata
    ...
    for epoch in epochs:
        ...
        # Extract evaluation representations
        eval_reps = [
        ...
        p_slsn = probs_eval[:, 2]  # Class 95 index is 2

        # Compute discrimination metrics
        res_kn = compute_bootstrap_auc(
            y_true=y_eval, y_prob=p_kn, target_class=64, n_bootstrap=1000, seed=42
        )
        res_slsn = compute_bootstrap_auc(
            y_true=y_eval, y_prob=p_slsn, target_class=95, n_bootstrap=1000, seed=42
        )
        ...
        print(
            f"  [Epoch {epoch:.1f}d] Kilonova ROC-AUC: {res_kn['roc_auc']:.4f} "
            f"[{res_kn['roc_auc_ci_low']:.4f}, {res_kn['roc_auc_ci_high']:.4f}] | "
            f"PR-AUC: {res_kn['pr_auc']:.4f} "
            f"[{res_kn['pr_auc_ci_low']:.4f}, {res_kn['pr_auc_ci_high']:.4f}]",
            flush=True,
        )
        print(
            f"  [Epoch {epoch:.1f}d] SLSN-I ROC-AUC: {res_slsn['roc_auc']:.4f} "
            f"[{res_slsn['roc_auc_ci_low']:.4f}, {res_slsn['roc_auc_ci_high']:.4f}] | "
            f"PR-AUC: {res_slsn['pr_auc']:.4f} "
            f"[{res_slsn['pr_auc_ci_low']:.4f}, {res_slsn['pr_auc_ci_high']:.4f}]",
            flush=True,
        )

    # Save to a temporary metrics JSON file
    out_metrics_path = Path("docs/results/discrimination_scaleup_metrics.json")
    with open(out_metrics_path, "w") as f:
        json.dump(results_by_epoch, f, indent=2)

    print(f"\nSaved metrics to {out_metrics_path}", flush=True)
