...
- Evaluation light-curve cohort (`plasticc_test_lightcurves_01.csv.gz` slice): **12,740 study class objects**, containing **241 SLSN-I objects** ($1.8917\%$).

Per Step 0 instructions, while $N_{\text{SLSN}} = 241$ provides over $120\times$ more target events than the kilonova sample ($N=2$), SLSN-I remains a minor class ($1.89\%$) within the evaluation stream. The evaluation methodology will incorporate exact Clopper-Pearson binomial confidence intervals (ADR 008) and jackknife stability checks to ensure statistical rigor.
...
Evaluation Population (Intersection with `plasticc_test_lightcurves_01.csv.gz`)
  └── Total Objects: 12,740
      ├── Type Ia Supernovae (class 90): 12,497 (98.0926%)
      ├── Kilonovae (class 64): 2 (0.0157%)
      └── SLSN-I (class 95): 241 (1.8917%)
```
...
| **Evaluation Cohort (`lightcurves_01` slice)** | 12,740 | 241 | 2 | 12,497 | 1.8917% |

---

## 4. Methodological Implications for Generalization Evaluation

1. **Increased Statistical Power relative to Kilonovae:** With $N_{\text{SLSN}} = 241$ positive objects, point estimates for $MHVER$ and policy trigger counts have substantially tighter confidence bounds than the $N_{\text{KN}} = 2$ kilonova evaluation.
...
3. **Evaluation Protocol Requirements:**
   ...
   - Perform leave-one-out / jackknife stability checks across the $N=241$ objects to report min/max metric bounds.
